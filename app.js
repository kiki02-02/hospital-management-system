...
function renderTable() {
    ...
    // Update Dashboard Count (New Patients Card)
    const countElement = document.querySelector('.cardBox .card:nth-child(2) .numbers');
    if (countElement) {
        countElement.innerText = patients.length;
    }

    calculateEarnings(); // Update Earnings on render
}

// ============================================
// CALCULATE EARNINGS (From Saved Bills)
// ============================================
function calculateEarnings() {
    let bills = JSON.parse(localStorage.getItem('hdms_bills')) || [];
    let today = new Date().toLocaleDateString('en-GB'); // Current Date: DD/MM/YYYY

    let todaysTotal = 0;
    bills.forEach(bill => {
        // If bill.date exists and matches today
        // (If date wasn't saved in older versions, treat as 0 or ignore)
        if (bill.date === today) {
            todaysTotal += bill.amount;
        }
    });

    // Update the Dashboard Card (4th Card = Earnings)
    const earningElement = document.querySelector('.cardBox .card:nth-child(4) .numbers');
    if (earningElement) {
        earningElement.innerText = "₹" + todaysTotal.toLocaleString('en-IN');
    }
}
